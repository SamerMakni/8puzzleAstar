class PuzzleSolver:
    ...
    def get_performance(self):
        return (f'{self._strategy} - Expanded Nodes: {self._strategy.num_expanded_nodes}')
    ...
    def get_solution(self):
        solution = ''
        count = 0
        for i in range(0, self._strategy.solution.__len__()):
            if (i > 0):
                solution += str(self._strategy.solution[i]) + '\n'
            count += 1
        if count == 1:
...
class Puzzle:
    ...
    def _generate_end_position(self):
        """
        Example end position in 3x3 puzzle
        [[1, 2, 3], [8, 0, 4], [7, 6, 5]]
        """
        end_position = [[1, 2, 3], [8, 0, 4], [7, 6, 5]]
        return end_position
    ...
    def _get_inversions_count(self):
        ...
        return inv_count - 1  # -1 because the blank space is not included in the inversions count
    ...
    def is_solvable(self):
        ...
        print('Inversions count:', inversions_count)
        blank_position = self._get_blank_space_row_counting_from_bottom()
        print('Blank position:', blank_position)
...
if __name__ == '__main__':
    puzzle = Puzzle([[5, 2, 1], [0, 8, 3], [7, 6, 4]])
